...
<div class="container py-4 py-xl-5" style="margin-left: 72px;padding-left: 43px;">
    ...
    <div class="row row-cols-1 row-cols-md-2">
        <div class="col" style="display: flex; justify-content: center; align-items: center;"><img class="rounded w-50 h-70 fit-cover" style="min-height: 300px;" src="{{$book["img"]}}"></div>
        <div class="col d-flex flex-column justify-content-center p-4">
            ...
            <div class="row">
                <div class="col"><form
                        method="POST"
                        action="/myBooks/updateRead/{{$book["id"]}}">
                        {!! csrf_field() !!}
                        <input
                            type="hidden"
                            name="_method"
                            value="PUT">
                        <?php
                          $buttonText = ($book["przeczytana"] === 'tak') ? 'przeczytane' : 'nieprzeczytane';
                          $buttonColor = ($book["przeczytana"] === 'tak') ? 'green' : 'blue';
                          ?>

                        <button class="btn btn-primary" type="submit" style="background: <?php echo $buttonColor; ?>;">
                            <?php echo $buttonText; ?>
                        </button>
                ...
                <form method="POST" action="/books/{{ $book['id'] }}" onsubmit="return confirm('Do you really want to delete?');">
                    @csrf
                    @method('DELETE')
                    <button class="btn btn-primary" type="submit" style="background: rgb(224,56,45);">usuń</button>
                </form>
                </div>
            </div>
        </div>
    ...
    <div class="row">
        ...
        <form
            ...
            <input
                ...
                name="_method"
                value="PUT">
            <h1 style="font-size: 20px;text-align: center;">dodaj notatke</h1><textarea name="notatka" style="background: rgb(188,231,241);width: 50%;padding-left: 0px;margin-left: 25%;margin-right: auto;" placeholder="zapisz cos" rows="5">{{$book['notatka']}}</textarea><button class="btn btn-primary" type="submit" style="width: 20%;margin-left: 40%;">Zapisz</button>
